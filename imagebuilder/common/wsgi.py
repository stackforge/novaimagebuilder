...
class AppFactory(BasePasteFactory):
    ...
    This requires imagebuilder.app_factory to be set to a callable which returns a
    WSGI app when invoked. The format of the name is <module>:<callable> e.g.

      [app:apiv1app]
      paste.app_factory = imagebuilder.common.wsgi:app_factory
      imagebuilder.app_factory = imagebuilder.api.v1:API

    The WSGI app constructor must accept a ConfigOpts object and a local config
    dict as its two arguments.
    """

    KEY = 'imagebuilder.app_factory'
...
class FilterFactory(AppFactory):
    ...
    This requires imagebuilder.filter_factory to be set to a callable which returns a
    WSGI filter when invoked. The format is <module>:<callable> e.g.

      [filter:cache]
      paste.filter_factory = imagebuilder.common.wsgi:filter_factory
      imagebuilder.filter_factory = imagebuilder.api.middleware.cache:CacheFilter

    The WSGI filter constructor must accept a WSGI app, a ConfigOpts object and
    a local config dict as its three arguments.
    """

    KEY = 'imagebuilder.filter_factory'
