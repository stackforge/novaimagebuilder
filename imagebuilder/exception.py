...
from imagebuilder import safe_utils
...
def wrap_exception(notifier=None, publisher_id=None, event_type=None,
    ...
    """
    # TODO(sandy): Find a way to import imagebuilder.notifier.api so we don't have
    # to pass it in as a parameter. Otherwise we get a cyclic import of
    # imagebuilder.notifier.api -> imagebuilder.utils -> imagebuilder.exception :(
    def inner(f):
...
class ImageNotFoundEC2(ImageNotFound):
    message = _("Image %(image_id)s could not be found. The imagebuilder EC2 API "
...
class QuotaClassNotFound(QuotaNotFound):
    message = _("Quota class uild of instance %(instance_uuid)s was re-scheduled: "
                "%(reason)s")
class QuotaResourceUnknown(QuotaNotFound):
    message = _("Unknown quota resources %(unknown)s.")
